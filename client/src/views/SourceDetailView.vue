<template>
  ...
  <!--
    ...
    - events:
        无
  -->
  <article v-if="record" class="source-detail">
    <!--
      [DEFAULT] ele(header.source-detail__header)
      - condition:
          record 存在时默认渲染。
      ...
      <div class="source-detail__heading-row">
        ...
        <div class="source-detail__primary-actions" aria-label="数据源主要操作">
          <!--
            ...
            - condition:
                默认渲染；isChecking 为 true 时显示加载状态。
            - type:
                第三方组件
                组件库: Element UI
                组件名称: el-button
            ...
            - events:
                @click
                    ...
                    - methods:
                        handleCheckSource()
          -->
          <el-button class="source-detail__check" :loading="isChecking" @click="handleCheckSource">检测</el-button>
          <!--
    ...
    <!--
      [IF isUnavailable] ele(el-alert)
      - condition:
          当前数据源已启用且健康状态为 unavailable 时渲染。
      - type:
          第三方组件
          组件库: Element UI
          组件名称: el-alert
      ...
      - params:
          -- unavailableReason：运行态最后不可用原因或稳定兜底文案。
      - events:
          无
    -->
    <el-alert
  ...
  <!--
    ...
    - events:
        @click 调用 returnToSourceList()。
  -->
  <section v-else class="source-detail-not-found theme-surface">
    <!--
...
/*
  ...
  - 文件职责:
      展示单个数据源的基本信息、普通设置边界、缓存占用和可执行操作。
      通过 settingsService 协调检测、更新、授权、默认源交接、导出和删除，不直接改写共享记录。
  ...
  - 模块级常量:
      ...
      UNAVAILABLE_FALLBACK_REASON: string，不可用原因缺失时的兜底文案。
  ...
  - 对外导出:
      SourceDetailView: 当前文件公开的组件或模块能力。
*/
...
// 类型: string。
// 作用: 健康状态不可用但没有原因时提供明确兜底说明。

const UNAVAILABLE_FALLBACK_REASON = '该数据源当前不可用，请稍后重新检测。';
 ...
 */
  ...
  computed: {
     ...
     * 纯函数: record 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
    record() {
      // 类型: string。
     ...
     * 纯函数: isDefaultSource 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
    isDefaultSource() {
     ...
     * 纯函数: isCustomSource 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
    isCustomSource() {
     ...
     * 纯函数: authorizationState 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
    authorizationState() {
      // 返回值类型: object。
 ...
 */
     ...
     * 纯函数: isChecking 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
    isChecking() {
    ...
    /**
     * 判断是否需要展示不可用原因提示。
     * 数据来源: record.runtime.enabled 和 healthStatus。
     * 只有已启用且健康状态不可用时返回 true，关闭状态不重复展示健康警告。
     *
     * @returns {boolean} 是否展示不可用提示。
     * 纯函数: isUnavailable 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
    isUnavailable() {
      return Boolean(
        this.record
        && this.record.runtime.enabled
        && this.record.runtime.healthStatus === HEALTH_STATUS.unavailable
      );
    },

    /**
     * 计算当前数据源不可用原因。
     * 数据来源: record.runtime.lastUnavailableReason。
     * 原因缺失时使用 UNAVAILABLE_FALLBACK_REASON，避免警示区域空白。
     *
     * @returns {string} 真实不可用原因或统一兜底文案。
     * 纯函数: unavailableReason 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
    unavailableReason() {
      // 条件分支: lastUnavailableReason 非空时返回真实原因，否则返回统一兜底。
      return this.record.runtime.lastUnavailableReason || UNAVAILABLE_FALLBACK_REASON;
    },

    /**
     * 判断当前记录是否允许设为默认源。
     * 数据来源: record.runtime.enabled 和 isDefaultSource。
     * 只有已启用且尚非默认源的记录返回 true。
     *
     * @returns {boolean} “设为默认”按钮是否可用。
     * 纯函数: canSetDefault 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
    canSetDefault() {
      return Boolean(this.record && this.record.runtime.enabled && !this.isDefaultSource);
    },

    /**
     * 计算默认源交接候选记录。
     * 数据来源: 全部可见记录和当前 record。
     * 排除当前记录与未启用记录，保证候选源可立即接替。
     *
     * @returns {Array<object>} 其他已启用可见数据源记录。
 ...
 */
     ...
     * 纯函数: defaultSourceOperationDescription 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
     ...
     * 纯函数: defaultSourceOperationConfirmLabel 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
     ...
     * 纯函数: requiresAuthorization 只读取输入参数或组件只读状态并返回派生结果，不修改响应式状态或外部存储。
 */
 ...
 */
  ...
  methods: {
     ...
     * 副作用: 通过 vue-router 返回数据源列表，并更新浏览器当前地址。
     */
    returnToSourceList() {
     ...
     * 副作用: handleCheckSource 会检测目标状态，并同步相关组件状态、路由或对外事件。
     * 成功路径: handleCheckSource 完成检测目标状态后同步成功结果。
     * 失败路径: 检测服务拒绝时 Promise 继续向调用方抛出，不显示成功消息。
 */
    async handleCheckSource() {
      // 异步操作: 根据当前记录 id 更新共享健康状态和检测时间。
      await checkSource(this.record.definition.id);
      // 成功反馈: 检测完成后通知用户。
      this.$message.success('数据源检测完成');
    },
     ...
     *
     * @returns {void} 该方法通过 service 修改共享状态。
     * 副作用: 调用 service 更新唯一默认源，并显示成功反馈。
     */
    handleSetDefault() {
      // 共享状态副作用: 使用当前记录 id 更新默认源。
     ...
     * 副作用: handleCheckUpdate 会检测目标状态，并同步相关组件状态、路由或对外事件。
     * 成功路径: handleCheckUpdate 完成检测目标状态后同步成功结果。
     * 失败路径: 更新检查服务拒绝时 Promise 继续向调用方抛出，不显示检查结果消息。
 */
    async handleCheckUpdate() {
      // 异步操作: 根据当前记录 id 更新远程版本检查状态。
      await checkSourceUpdate(this.record.definition.id);
     ...
     * 副作用: 默认自定义源先打开交接弹窗，其他记录直接委托脚本更新流程。
     * 成功路径: 交接弹窗成功打开或脚本更新完成后兑现。
     * 失败路径: 脚本更新服务拒绝时 Promise 继续向调用方抛出。
 */
     ...
     *
     * @returns {Promise<void>} 更新完成后兑现。
     * 副作用: 调用 service 写入脚本更新事实，并根据来源类型显示授权或完成提示。
     * 成功路径: 脚本更新应用完成并显示对应成功消息。
     * 失败路径: 更新服务拒绝时 Promise 继续向调用方抛出，不显示成功消息。
 */
    async performApplyUpdate() {
      // 异步操作: 根据当前记录 id 应用在线脚本 mock 更新。
      await applySourceUpdate(this.record.definition.id);
     ...
     * 触发来源: 用户点击“导出脚本”按钮。
     *
     * @returns {void} 下载由浏览器处理，不返回业务数据。
     * 副作用: handleExportScript 会导出数据源脚本，并同步相关组件状态、路由或对外事件。
     */
    handleExportScript() {
      // 浏览器副作用: 根据当前记录 id 生成并下载脚本文件。
     ...
     * 副作用: handleRevokeAuthorization 会更新脚本授权，并同步相关组件状态、路由或对外事件。
     * 成功路径: handleRevokeAuthorization 完成更新脚本授权后同步成功结果。
     * 失败路径: 用户取消确认时保持授权不变；非取消异常继续抛给上层处理。
 */
     ...
     *
     * @returns {void} 该方法通过 service 修改共享缓存状态。
     * 副作用: handleClearTemporaryCache 会清理目标数据，并同步相关组件状态、路由或对外事件。
     */
    handleClearTemporaryCache() {
      clearTemporarySourceCache(this.record.definition.id);
      this.$message.success('临时缓存已清理');
    },

    /**
     * 清理当前数据源全部运行缓存。
     * 触发来源: SourceCacheSection 的 clear-all 事件。
     * 用户确认后清理请求头、Cookie、身份令牌、会话和验证上下文等缓存。
     * 用户取消属于正常退出，其他异常继续抛出。
     *
     * @returns {Promise<void>} 确认和清理流程结束后兑现。
     * 副作用: handleClearAllCache 会清理目标数据，并同步相关组件状态、路由或对外事件。
     * 成功路径: handleClearAllCache 完成清理目标数据后同步成功结果。
     * 失败路径: 用户取消确认时保持缓存不变；非取消异常继续抛给上层处理。
 */
    async handleClearAllCache() {
      try {
        // 异步交互: 等待用户确认全部缓存清理边界和后续可能重新验证的影响。
        await this.$confirm(
          '该操作会清理请求头运行数据、Cookie、身份令牌、会话和验证上下文等全部缓存，后续请求可能需要重新验证。数据源脚本、基本信息、启用状态、默认源和运行授权会保留。',
          '清理全部缓存',
          { type: 'warning', confirmButtonText: '清理全部缓存', cancelButtonText: '取消' }
        );
        // 共享状态副作用: 用户确认后清理当前记录全部运行缓存。
        clearAllSourceCache(this.record.definition.id);
        this.$message.success('全部缓存已清理');
      } catch (action) {
        // 用户取消确认时不执行缓存清理，也不把正常取消显示为错误。
        // 条件分支: action 不是 cancel 或 close 时继续抛出真实异常。
        // 执行内容: 正常取消保持缓存不变，非取消异常交给上层错误边界处理。
        if (action !== 'cancel' && action !== 'close') throw action;
      }
    },
 ...
 */
     ...
     * @returns {void} 该方法通过 service 修改共享状态并触发路由导航。
     * 副作用: performDelete 会删除目标记录，并同步相关组件状态、路由或对外事件。
 */
     ...
     * 副作用: openDefaultSourceTransition 会打开目标页面或弹窗，并同步相关组件状态、路由或对外事件。
 */
     ...
     * 有候选源时先切换 defaultSourceId，再按 pendingDefaultSourceAction 执行唯一后续分支。
     * 未知或已清空动作不会执行额外业务修改。
     *
     * @param {{ sourceId: string, fallbackSourceId: string }} payload 默认源交接结果。
     * @returns {Promise<void>} 等待可能存在的 mock 更新完成。
 * @param {*} payload 当前函数接收的 payload 参数。
 * 副作用: confirmDefaultSourceTransition 会提交当前交互，并同步相关组件状态、路由或对外事件。
 * 成功路径: confirmDefaultSourceTransition 完成提交当前交互后同步成功结果。
 * 失败路径: confirmDefaultSourceTransition 沿用现有 catch、finally 或调用方错误边界记录失败并结束加载状态。
 */
    async confirmDefaultSourceTransition(payload) {
      // 条件分支: 用户选择了候选默认源时进入。
      // 执行内容: 在继续原操作前先写入新默认源，保持默认源连续性。

      if (payload.fallbackSourceId) {
        setDefaultSource(payload.fallbackSourceId);
      }
      ...
      // 类型: string。
      // 作用: 立即清除待处理动作，避免重复确认再次执行同一操作。
      this.pendingDefaultSourceAction = '';

      // 条件分支: pendingAction 为 update 时进入。
      // 执行内容: 等待脚本更新完成后结束，避免继续命中后续动作分支。
      ...
      // 条件分支: pendingAction 为 revokeAuthorization 时进入。
      // 执行内容: 撤销原默认源授权并同步关闭，然后显示成功消息。

      if (pendingAction === DEFAULT_SOURCE_ACTION.revokeAuthorization) {
        revokeSourceAuthorization(payload.sourceId);
        this.$message.success('已撤销运行授权并关闭数据源');
        return;
      }

      // 条件分支: pendingAction 为 deleteSource 时进入。
      // 执行内容: 删除原默认源，并由 performDelete 返回列表页。

      if (pendingAction === DEFAULT_SOURCE_ACTION.deleteSource) {
        this.performDelete(payload.sourceId);
      }
    }
  }
};
  ...
  */
  .source-detail__danger-zone {
    ...
    padding: 17px 14px;
      ...
      // 三目条件: 删除前记录是否为系统源。
      // true 分支: 提示系统源已隐藏且可以恢复。
      // false 分支: 提示自定义数据源已经实际删除。
