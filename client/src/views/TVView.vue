...
export default {
  ...
  computed: {
    /**
     * 当前电视剧页筛选状态。
     * 数据来源: 当前路由 query 和 DEFAULT_TV_FILTER_SELECTION。
     * 纯函数: 只解析 URL 请求事实，不修改 Router、组件状态或筛选 Store。
     * 失败路径: 缺失或非法 query 字段回退默认筛选。
     *
     * @returns {object} 当前电视剧请求使用的筛选对象。
     */
    selectedFilters() {
      // 类型: object；作用: 从 URL 派生当前筛选，避免 KeepAlive 期间维护第二份可漂移状态。
      return createCatalogRouteState(this.$route.query, DEFAULT_TV_FILTER_SELECTION).filters;
    },
     ...
     */
    requestedPage() {
      // 类型: number；作用: 刷新和浏览器前进/后退时恢复同一电视剧目录页码。
      return createCatalogRouteState(this.$route.query, DEFAULT_TV_FILTER_SELECTION).page;
    },
   ...
   */
  created() {
    // 类型: Readonly<object>；作用: 当前 TVView 实例独享的路由请求身份守卫。
    this._routeRequestGuard = createRouteRequestGuard({ routeNames: ['tv'] });
    // 副作用: 把首次 URL 标记为已处理，返回同一 KeepAlive 地址时不重复请求。
    this._routeRequestGuard.markHandled(this.$route);

    // 执行内容: 首次进入电视剧页时并行请求筛选元数据和当前 URL 页码的内容列表。
    // 影响范围: 请求成功后电视剧筛选数据桶和电视剧内容数据桶会更新，页面通过 filterStore、getBucketItems('tv') 和 getPagePagination('tv') 读取。
    this.loadInitialTVPage(this.requestedPage);
  },

  watch: {
    /**
     * 监听电视剧页请求 URL 变化。
     * 触发来源: 筛选、分页、浏览器前进/后退或其他代码修改 tv query。
     * 副作用: 只有当前 TVView 负责的新 fullPath 才重新请求内容；失活 KeepAlive 实例直接跳过。
    ...
    '$route.fullPath'() {
      ...
      // 异步调用: 只按新 URL 页码请求电视剧内容；筛选值由 computed 从同一 URL 派生。
      this.loadTVContent(this.requestedPage);
    }
  },
  ...
  methods: {
     ...
     */
    async loadInitialTVPage(page = 1) {
      // 类型: boolean。
      ...
      try {
        ...
        await Promise.all([
          ...
          requestSourceData(this.createTVPageRequest(page))
        ]);
      } catch (error) {
        // 类型: string。
    ...
    async handleSourceSwitched() {
      ...
      const targetLocation = {
        ...
        query: createCatalogRouteQuery({
          ...
          defaults: DEFAULT_TV_FILTER_SELECTION,
          filters: DEFAULT_TV_FILTER_SELECTION,
          page: 1
        })
      };
