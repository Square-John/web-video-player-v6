<template>
  <!--
    ...
    └─ {div.section-body}
       ├─ {div.section-grid}
       │  ├─ [if hasMovies]
       │  │  └─ {VideoCard} 循环渲染 displayMovies，最多显示 10 张
       │  └─ [else]
       │     └─ {el-empty} 电影卡片分区空状态
       └─ {aside.section-aside}
...
export default {
  ...
  computed: {
     ...
     * 首页实际展示的电影卡片。
     *
     * @returns {Array<object>} 最多 10 条电影数据，用于固定两行五列。
     */
    displayMovies() {
      // 首页卡片区只承担概览职责，多出来的数据留给电影页列表承接。
      return Array.isArray(this.movies) ? this.movies.filter(Boolean).slice(0, 10) : [];
    }
  }
};
</script>
...
/*
  ...
  样式作用:
  建立左侧五列电影卡片和右侧电影排行榜的 7 列布局。
  通过 CSS Grid 的行高拉伸，让右侧排行榜跟随左侧两行电影卡片真实高度。
  让电影排行榜贴合主体栅格最右列，右侧留白统一交给页面容器处理。
*/
.section-body {
  /* 设置热门电影主体为 CSS Grid，用 7 列承载左侧卡片区和右侧排行榜。 */
  display: grid;

  /* 设置热门电影主体拆成 7 等份，左侧电影卡片占 5 列，右侧排行榜占 2 列。 */
...
*/
.section-grid {
  /* 设置电影卡片区内部为 grid，方便把 10 张卡片排成两行五列。 */
  display: grid;

  /* 设置电影卡片区占据首页 7 列栅格中的前 5 列。 */
  grid-column: span 5;

  /* 设置电影卡片区内部为 5 列，让热门电影首屏形成两行五列。 */
