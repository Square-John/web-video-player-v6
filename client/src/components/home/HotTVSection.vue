<template>
  <!--
    ...
    │  - description:
    │      首页热门电视剧区块。
    │      左侧展示 Provider 返回的当前页电视剧卡片，右侧展示电视剧排行榜。
    │  - params:
    │      -- tvList：父组件传入的首页热门电视剧列表。
    │      -- ranking：父组件传入的电视剧排行榜列表。
    │      -- pagination：hotTv 数据桶的标准分页信息。
    │      -- paging：hotTv 数据桶是否正在请求。
    ...
    │              用于通知 HomeView 进入电视剧承接页。
    │          - methods:
    │              handleOpenMoreRanking(rankingKey)
    │                  -- rankingKey：固定电视剧排行榜区域标识。
    │      @change-page
    │          - description: 用户点击标题栏相邻分页按钮时通知 HomeView 请求电视剧目标页。
    │          - methods: handleChangePage(targetPage)，补充 hotTv 模块身份后派发。
    │
  ...
  <section class="section-wrapper">
    <!--
      [DEFAULT] ele(div.section-body)
      - condition: 热门电视剧区块始终渲染主体 Grid。
      - type: 原生 div。
      - description: 用两行六列统一编排标题栏、左下卡片和右下排行榜。
      - params: -- hasTVList/displayTVList/ranking：分别驱动卡片、空态和排行榜。
...
/*
  ...
  - 模块级常量:
      ...
      HOT_TV_MODULE_KEY: string，标识热门电视剧列表 PageBucket 与分页事件目标。
  ...
  - 对外导出:
      HotTVSection: Vue component，供 HomeView 渲染热门电视剧概览和转发区块交互。
*/
...
// 类型: string；来源: 首页 PageBucket 定位契约；作用: 给分页事件补充热门电视剧区域身份。
const HOT_TV_MODULE_KEY = 'hotTv';
...
export default {
  ...
  components: {
    // UserVideoCard 负责单张电视剧卡片的内容展示，并注入用户收藏和播放状态。
    UserVideoCard,
  ...
  props: {
    ...
    // 类型: object。
    // 来源: HomeView 从 pages.home.hotTv.pagination selector 读取。
    ...
    // 类型: boolean。
    // 来源: HomeView 从 pages.home.hotTv.transaction.status 派生。
    // true: 热门电视剧正在请求，分页按钮禁用。
  ...
  computed: {
     ...
     *
     * @returns {string} HomeView 可识别的 tvRanking 模块标识。
     */
    rankingKey() {
      return TV_RANKING_KEY;
    },

    /**
     * 左侧电视剧卡片区是否有数据。
     * 纯函数: 只读取 displayTVList，不修改组件、Store 或传入数组。
     *
     ...
     *
     * @returns {Array<object>} Provider 当前页返回的有效电视剧数据。
     */
    displayTVList() {
      // 返回值类型: Array<object>；作用: 保留当前远程页全部有效条目，总量边界由 Provider 分页响应决定。
      return Array.isArray(this.tvList) ? this.tvList.filter(Boolean) : [];
    }
  },
  ...
  methods: {
     ...
     * 副作用: 只派发带 hotTv 身份的 change-page，不请求网络或修改 pagination prop。
     * 失败路径: 非正整数目标页不派发，避免页面收到非法请求。
     *
     * @param {number} targetPage 用户点击得到的相邻目标页码。
     * @returns {void} 有效时通知 HomeView 请求热门电视剧目标页。
     */
    handleChangePage(targetPage) {
      ...
      // 事件: change-page；参数: object，包含热门电视剧模块身份和目标页码。
      this.$emit('change-page', {
        moduleKey: HOT_TV_MODULE_KEY,
        page: targetPage
      });
    },
...
*/
.section-grid {
  /* 设置电视剧卡片区内部为 grid，让当前远程页条目按统一列数自然换行。 */
  display: grid;

  /* 设置电视剧卡片区占据第二行前 4 列，与标题栏共享左右列线。 */
  grid-column: 1 / span 4;

  /* 电视剧卡片固定进入第二行，让排行榜顶部与首排卡片对齐。 */
  grid-row: 2;
...
*/
.section-aside {
  /* 设置电视剧排行榜列占据第二行最后 2 列，不侵入标题栏所在第一行。 */
  grid-column: 5 / span 2;

  /* 排行榜固定进入第二行，使顶部与首排电视剧卡片一致。 */
  grid-row: 2;
